	...
	Describe("KServe KEDA Reconciler", func() {
		...
		makeKedaTestISVC := func(namespace, name string, enableKedaMetrics bool) *kservev1beta1.InferenceService {
				...
				Spec: kservev1beta1.InferenceServiceSpec{
					Predictor: kservev1beta1.PredictorSpec{
						...
						Model: &kservev1beta1.ModelSpec{
							...
							Runtime:     pointer.String("kserve-ovms"),
							PredictorExtensionSpec: kservev1beta1.PredictorExtensionSpec{
								StorageURI: pointer.String("s3://modelmesh-example-models/onnx/mnist-8"),
							},
						},
					},
				},
			...
			if enableKedaMetrics {
				isvc.Spec.Predictor.MinReplicas = pointer.Int32(1)
		...
		Context("when a InferenceServices are configured to scale on KEDA metrics", func() {
...
`, kind, obj, isvc2))
			...
			It("Must remove owner reference from deleted InferenceService", func() {
				...
				Expect(kedaReconciler.Delete(ctx, GinkgoLogr, isvc2)).To(BeNil())
...
`, kind, obj, isvc2))
			...
			AfterEach(func() {
				...
				Expect(kedaReconciler.Delete(ctx, GinkgoLogr, isvc)).To(BeNil())
				Expect(kedaReconciler.Cleanup(ctx, GinkgoLogr, testNs)).To(BeNil())

				Expect(getAllKedaTestResources(ctx, testNs)).To(HaveLen(0))
			})
		})
		...
		Context("when an InferenceService no longer requires KEDA metrics", func() {
			...
			BeforeEach(func() {
				...
				// Create KEDA resources owned by this ISVC
				_ = createTestKedaSA(ctx, testNs, isvc, nil)
				_ = createTestKedaSecret(ctx, testNs, isvc, nil)
				_ = createTestKedaRole(ctx, testNs, isvc, nil)
				_ = createTestKedaRoleBinding(ctx, testNs, isvc, nil)
				_ = createTestKedaTA(ctx, testNs, isvc, nil)
...
`, kind, obj, isvc))
		...
		Context("when KEDA resources do not exist and autoscaling is not configured", func() {
			...
			BeforeEach(func() {
				...
				Expect(k8sClient.Create(ctx, isvc)).Should(Succeed())
				// Ensure no KEDA resources are created for this test
			})
		...
		Context("when KEDA resource does not have the target ISVC as an owner", func() {
			var isvc *kservev1beta1.InferenceService
